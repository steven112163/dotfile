...
# via venv.sh's venv_generate_so_aliases. uv.sh is sourced after venv.sh, so a
# venv and a uv-venv sharing a basename silently resolve to this uv alias
# (venv_generate_so_aliases logs an overwrite diagnostic when that happens).
generate_uv_venv_aliases() {
    venv_generate_so_aliases "$MY_UV_VENV_ROOT"
}
